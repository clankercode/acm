...
| **cursor-agent** (CLI) | `~/.cursor/projects/*/agent-transcripts/*/*.jsonl` | JSONL transcripts | **Negative — usage not persisted.** The stream-json CLI output emits per-request `usage: {inputTokens, outputTokens, cacheReadTokens, cacheWriteTokens}` in the `result` event, but transcripts store only `{role, message}` — no usage, model, or timestamp. 552 transcript files, all conversation-only. | Would need a live-capture wrapper (intercept stream-json) since no usage is written to disk. Distinct from Cursor IDE.
...
6. **cursor-agent CLI** — distinct from Cursor IDE. The CLI emits `usage: {inputTokens, outputTokens, cacheReadTokens, cacheWriteTokens}` in its `--output-format stream-json` result event, but **never persists it** — the 552 transcripts under `~/.cursor/projects/*/agent-transcripts/` store only `{role, message}`. A CCM source would need a live-capture wrapper, not a history reader. **Verdict: negative — no persistent token store.**
